...
function detectCategory(text:string){ const t=(text||'').toLowerCase(); let best='overview', score=0; for(const c of CATEGORIES){ let s=0; for(const k of c.kw){ if(t.includes(k)) s++ } if(s>score){ score=s; best=c.key } } if(score===0){ const nums=(t.match(/(\d+[\.,]?\d*\s?(mm|cm|m|kg|w|v|hz|db|l|m³\/h|m3\/h))/gi)||[]).length; if(nums>=2) return 'specs' } return best }
